...
const AddPatientOnline = (props) => {
    ...
    const handleSendPatientDataButton = () => {
        ...
        if (!patientData.name) {
            ...
            console.log(patientData.symptoms.length)
            if (!nameRegex.test(patientData.name)) {
                ...
                if (patientData.image) {
                    ...
                    formData.append('name', patientData.name),
                        ...
                        formData.append('phNo', patientData.phNo),
                        formData.append('district', patientData.district),
                        formData.append('homeTown', patientData.homeTown),
                        formData.append('dengueStatus', patientData.dengueStatus),
                        formData.append('recommendedHospital', patientData.hospital),
                        formData.append('recommendedBy', patientData.recommendedBy),
                        formData.append('caseStatus', 'newForDoctor'),
                        formData.append('symptoms', patientData.symptoms),
                        formData.append('dispensary', patientData.dispensary),
                        formData.append('date', patientData.date),
                        formData.append("symptomsImage", {
                            uri: patientData.image.path,
                            name: moment().format("DDMMYYYhmmss") + 'symptomsImage.jpg',
                            type: 'image/jpeg',
                        });
                    ...
                    fetch('http://10.0.2.2:3000/addPatient', {
                        ...
                        body: JSON.stringify({
                            ...
                            'dengueStatus': patientData.dengueStatus,
                            'recommendedHospital': patientData.hospital,
                            'recommendedBy': patientData.recommendedBy,
                            'caseStatus': 'newForDoctor',
                            'symptoms': patientData.symptoms,
                            'dispensary': patientData.dispensary,
                                ...
                                >
                                    {!isPatientAdded ?
                                        <ScrollView showsVerticalScrollIndicator={false}>
                                            ...
                                            <View>
                                                <DropDownList
                                                    dropDownItems={hospitals.map(item => ({
                                                        value: item.name,
                                                        label: item.name
                                                    }))}
                                                    // dropDownDefaultValue={patientData.hospital}
                                                    dropDownWidth={textInputWidth}
                                                    dropDownPlaceHolder="hospital"
                                                    searchable={true}
                                                    dropDownBackgroundColor={commonStyles.textInputColor.color}
                                                    dropDownOnChangeItem={item => setPatientData((prevState) => { return { ...prevState, hospital: item.value } })}
...
export default AddPatientOnline;
